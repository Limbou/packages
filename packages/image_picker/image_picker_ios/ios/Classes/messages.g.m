  ...
  }
  {
    ...
    if (api) {
      ...
      [channel setMessageHandler:^(id _Nullable message, FlutterReply callback) {
        ...
        NSNumber *arg_requestFullMetadata = GetNullableObjectAtIndex(args, 2);
        [api pickVideoWithSource:arg_source maxDuration:arg_maxDurationSeconds fullMetadata:arg_requestFullMetadata completion:^(NSString *_Nullable output, FlutterError *_Nullable error) {
          callback(wrapResult(output, error));
        }];
      }];
    } else {
